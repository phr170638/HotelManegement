...
const summaryStats = computed(() => [
  ...
  { label: '限制登录', value: users.value.filter((item) => item.status !== 1).length }
])

function isAdminUser(row) {
  return getRoleMeta(row).key === 'admin'
}

function getRoleMeta(row) {
  if (row?.roleType === 'admin' || (Array.isArray(row?.roles) && row.roles.includes('admin'))) {
    return { key: 'admin', label: row?.roleDisplayName || '管理员', tagType: 'warning' }
  }
  if (row?.roleType === 'user' || (Array.isArray(row?.roles) && row.roles.includes('user'))) {
    return { key: 'user', label: row?.roleDisplayName || '普通用户', tagType: 'info' }
  }
  return { key: 'unknown', label: row?.roleDisplayName || '身份待同步', tagType: 'danger' }
}
...
async function toggleStatus(row) {
  if (isAdminUser(row)) {
    ElMessage.warning('管理员账号不支持在这里启用或禁用')
    return
  }
  ...
  <div class="admin-users">
    ...
    <section class="table-panel soft-card">
      <div class="toolbar">
        <div>
          <h3>用户管理</h3>
          <p>支持查看用户信息，并对普通用户账号状态进行恢复登录或限制登录管理。</p>
        </div>
        <div class="toolbar-actions">
          <el-button @click="fetchUsers">重新加载</el-button>
        </div>
      </div>
      ...
      <el-table :data="users" v-loading="loading">
        ...
        <el-table-column label="角色" width="140">
          <template #default="{ row }">
            <el-tag :type="getRoleMeta(row).tagType">
              {{ getRoleMeta(row).label }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="140">
          <template #default="{ row }">
            <el-button
              v-if="getRoleMeta(row).key === 'user'"
              size="small"
              :type="row.status === 1 ? 'danger' : 'success'"
              @click="toggleStatus(row)"
            >
              {{ row.status === 1 ? '限制登录' : '恢复登录' }}
            </el-button>
            <span v-else class="row-note">{{ isAdminUser(row) ? '管理员不可操作' : '角色未同步，暂不可操作' }}</span>
          </template>
        </el-table-column>
      </el-table>
